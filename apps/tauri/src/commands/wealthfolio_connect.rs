...
pub async fn store_sync_session(
    ...
    let store = shared_secret_store();
    match refresh_token.as_deref().map(str::trim) {
        Some(token) if !token.is_empty() => {
            if let Err(e) = store.set_secret(SYNC_REFRESH_TOKEN_KEY, token) {
                error!("Failed to store refresh token: {}", e);
                return Err(format!("Failed to store refresh token: {}", e));
            }
            let _ = store.delete_secret(SYNC_ACCESS_TOKEN_KEY);
            debug!("Refresh token stored successfully");
        }
        _ => {
            if let Err(e) = store.delete_secret(SYNC_REFRESH_TOKEN_KEY) {
...
pub async fn clear_sync_session(state: State<'_, Arc<ServiceContext>>) -> Result<(), String> {
    let store = shared_secret_store();
    let _ = store.delete_secret(SYNC_ACCESS_TOKEN_KEY);
    let refresh_result = store.delete_secret(SYNC_REFRESH_TOKEN_KEY);
