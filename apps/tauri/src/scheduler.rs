...
/// holdings recalculation is triggered so every existing TRANSACTIONS-mode
/// account gets its lot rows written. HOLDINGS-mode accounts (which skip
/// the activity-replay path) are backfilled from their latest non-calculated
/// snapshot.
///
/// Idempotent — subsequent launches see a non-empty table and exit early.
pub async fn backfill_lots_if_needed(context: &std::sync::Arc<ServiceContext>) {
    ...
    let count = match context.lots_repository.count_lots() {
        Ok(n) => n,
        Err(e) => {
            log::warn!("Lot backfill skipped: could not count lots ({})", e);
            return;
        }
    };
    ...
    log::info!("Lots table is empty — running full holdings recalculation to populate it");
    if let Err(e) = context
        .snapshot_service
        .recalculate_holdings_snapshots(None, SnapshotRecalcMode::Full)
        .await
    {
        log::warn!("Lot backfill recalculation failed: {}", e);
    }
    ...
    if let Err(e) = context
        ...
        .await
    {
        log::warn!("HOLDINGS lot backfill failed: {}", e);
    }
}
