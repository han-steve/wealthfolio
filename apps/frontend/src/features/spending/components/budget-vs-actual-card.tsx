...
export interface BudgetCategoryRow {
  ...
  difference: number; // budgeted - actual (negative when over)
}
...
export function BudgetVsActualCard({ budgetData, currency, isLoading }: Props) {
  if (isLoading) {
    return (
      <Card>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className={cn(isOverBudget ? "text-destructive" : "text-success")}>
              ...
              <PrivacyAmount value={Math.abs(spending.difference)} currency={currency} />
            </span>
          </div>
        </div>
