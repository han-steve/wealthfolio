...
function getEventYearRange(events: EventSpendingSummary[]): { minYear: number; maxYear: number } {
  ...
  if (events.length === 0) return { minYear: currentYear, maxYear: currentYear };
...
export function EventTimeline({
  ...
  const displayMode = useMemo<TimelineDisplayMode>(
    () => (maxYear - minYear >= 1 ? "year-blocks" : "month"),
    [minYear, maxYear],
  );
  ...
  const { rangeStart, rangeEnd, months, yearColumns, totalColumns } = useMemo(() => {
    if (displayMode === "year-blocks") {
      ...
      for (let year = minYear; year <= maxYear; year++) {
        yearCols.push({ year, label: String(year), index: year - minYear });
      }
      return {
  ...
  const maxLane = useMemo(
    () => timedEvents.reduce((max, e) => Math.max(max, e.lane), 0),
    [timedEvents],
  );
  ...
  const positionedEvents = useMemo(() => {
    return timedEvents.map((event) => {
      ...
      return { ...event, leftPx, widthPx, isNarrow };
    });
  ...
  const handleEventClick = (eventId: string) => {
    ...
    if (isMobile && newSelection) setIsMobileDetailsOpen(true);
  };
        ...
        <div className="text-right">
          <div
            className={cn(
              ...
              selectedEvent.totalSpending < 0 ? "text-green-600" : "text-red-600",
            )}
          >
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {categoryChartData.length > 0 && (
          <div className="space-y-2">
            <h4 className="text-muted-foreground text-sm font-medium">Spending by Category</h4>
                      ...
                      return (
                        <div className="bg-background rounded-lg border p-2 shadow-sm">
                          <div className="text-muted-foreground text-xs">
                            {(p.payload as { dateLabel: string }).dateLabel}
                          </div>
                          <div
                            className="text-sm font-medium"
                            style={{ color: selectedEvent.eventTypeColor || "var(--chart-1)" }}
                          >
                            <PrivacyAmount
  ...
  return (
    <TooltipProvider>
      <div className="space-y-4">
        ...
        <div
          ref={containerRef}
          className="relative hidden overflow-x-auto rounded-lg border md:block"
        >
          <div
            className="relative"
            style={{
          ...
          >
            ...
            <div
              ...
              style={{ top: `${headerHeight + 8}px`, height: `${timelineHeight - 16}px` }}
            >
              {positionedEvents.map((event) => {
                const color = event.eventTypeColor || "var(--chart-1)";
                const isSelected = selectedEventId === event.eventId;
                const barContent = (
                  <button
                    key={event.eventId}
                  ...
                  >
                    ...
                    />
                    {!event.isNarrow && (
                      <div className="flex min-w-0 flex-1 items-center justify-between overflow-hidden pl-2 pr-1">
                        ...
                        <span className="text-foreground ml-1 whitespace-nowrap text-xs font-medium">
                          <PrivacyAmount value={event.totalSpending} currency={event.currency} />
                        </span>
                      </div>
                    )}
        ...
        {selectedEvent && !isMobile && (
          <div className="bg-muted/30 hidden rounded-lg border p-4 md:block">{detailPanel}</div>
        )}
        ...
        <Sheet open={isMobileDetailsOpen} onOpenChange={setIsMobileDetailsOpen}>
          <SheetContent side="bottom" className="max-h-[85vh] overflow-y-auto">
            <SheetHeader>
              <SheetTitle>{selectedEvent?.eventName ?? "Event"}</SheetTitle>
            </SheetHeader>
