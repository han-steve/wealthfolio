...
import { periodToDateRange, type SpendingPeriod } from "../lib/navigation";
...
export default function SpendingEventsVizPage() {
  ...
  const totalSpending = useMemo(
    () => filteredSummaries.reduce((sum, s) => sum + s.totalSpending, 0),
    [filteredSummaries],
  );

  const totalTransactions = useMemo(
    () => filteredSummaries.reduce((sum, s) => sum + s.transactionCount, 0),
    [filteredSummaries],
  );

  const baseCurrency = filteredSummaries[0]?.currency ?? "USD";
  ...
  if (error) return <div className="p-4">Failed to load events: {error.message}</div>;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col space-y-6 px-2 pb-2 pt-2 lg:px-4 lg:pb-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Per-event spending — trips, holidays, special occasions.
          </p>
        </div>
      ...
      {/* Events by Type bar chart + Category treemap */}
        ...
        <Card>
          ...
          <CardContent>
            {eventTypeChartData.length > 0 ? (
              <ChartContainer config={{ count: { label: "Events" } }} className="h-[200px] w-full">
                <BarChart
                ...
                >
                  ...
                  <ChartTooltip
                    content={({ active, payload }) => {
                      ...
                      const data = (payload[0] as { payload: (typeof eventTypeChartData)[number] })
                        .payload;
                      return (
