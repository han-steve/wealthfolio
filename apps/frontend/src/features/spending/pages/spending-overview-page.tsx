...
import { getEventsWithNames, getSpendingSummary } from "../adapters/summary";
import { SpendingHistoryChart } from "../components/spending-history-chart";
import { buildCashflowUrl, periodToDateRange, type SpendingPeriod } from "../lib/navigation";
import type { SpendingSummary, SubcategorySpending } from "../types";
...
type SelectedPeriod = "TOTAL" | "YTD" | "LAST_YEAR";
...
const SpendingPeriodSelector: React.FC<{
  ...
  onPeriodSelect: (period: SelectedPeriod) => void;
...
export default function SpendingOverviewPage() {
  ...
  const [selectedPeriod, setSelectedPeriod] = useState<SelectedPeriod>("TOTAL");
  ...
  const { data: events = [] } = useQuery<EventWithTypeName[], Error>({
    queryKey: [QueryKeys.SPENDING_EVENTS, "with-names"],
  ...
  const includeEventIds = useMemo(() => {
    ...
    selectedEventValues.forEach((v) => {
      if (v !== INCLUDE_ALL_EVENTS_VALUE) ids.push(v);
    });
  ...
  const filteredTotals = useMemo(() => {
    if (!periodSummary) return { totalSpending: 0, monthlyAverage: 0 };
    ...
    Object.entries(periodSummary.byCategory).forEach(([categoryId, cat]) => {
      if (!hiddenCategories.has(categoryId)) filteredSpending += cat.amount;
    });
  ...
  if (isLoading) return <SpendingDashboardSkeleton />;
  ...
  const { totalSpending, monthlyAverage } =
    hiddenCategories.size > 0 || hiddenSubcategories.size > 0
      ? filteredTotals
      : {
  ...
  const getPreviousPeriodData = (currentMonth: string): number => {
    ...
    return Number(totalSummary.byMonth[previousYearMonth]) || 0;
  };
  ...
  const previousMonthlyAverage =
    previousMonthlySpendingData.length > 0
      ? previousMonthlySpendingData.reduce((sum, [, value]) => sum + (Number(value) || 0), 0) /
        previousMonthlySpendingData.length
      : 0;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col space-y-6 px-2 pb-2 pt-2 lg:px-4 lg:pb-4">
      {/* Header: title + period selector + events filter */}
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Spending</h1>
          <p className="text-muted-foreground text-sm">
            Cash outflow across your tracked spending accounts.
          </p>
        </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
              {topSubcategories.map(([key, sub], index) => {
                ...
                return (
                  <div
                    key={key}
                    className={`bg-muted/30 hover:bg-muted/50 flex flex-col gap-1 rounded-lg border p-3 transition-colors ${
                      isHidden || isParentHidden ? "opacity-50" : ""
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <button
                    ...
                    <span
                      className="text-muted-foreground cursor-pointer truncate pl-[26px] text-xs"
                      onClick={() => handleSubcategoryClick(sub.subcategoryId)}
                    >
                      {sub.categoryName}
                    </span>
                    <div
                      className="mt-1 flex cursor-pointer items-center justify-between pl-[26px]"
                      onClick={() => handleSubcategoryClick(sub.subcategoryId)}
                    >
                      <span className="text-muted-foreground text-xs">
                        {percentage.toFixed(1)}%
