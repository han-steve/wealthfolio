...
/// Snapshot holdings filter: always include manually-entered/imported snapshots.
/// Also include any CALCULATED snapshot from the past 24 hours so a newly-bootstrapped
/// device immediately has cached portfolio values and doesn't show $0.00 while waiting
/// for its own background portfolio recalculation to finish.
const USER_SYNCABLE_HOLDINGS_SNAPSHOTS_FILTER: &str =
    "source IN ('MANUAL_ENTRY', 'CSV_IMPORT', 'SYNTHETIC', 'BROKER_IMPORTED') \
     OR (source = 'CALCULATED' AND calculated_at >= datetime('now', '-1 day'))";
/// Snapshot quotes filter: include ALL manual quotes (bonds, money-market daily
/// prices) plus the most recent 7 days of auto-fetched quotes so that a
/// newly-bootstrapped device has current market prices immediately, rather than
/// showing $0.00 until it can fetch from Yahoo on its own.
const SYNCABLE_QUOTES_FILTER: &str = "source = 'MANUAL' \
     OR day >= date('now', '-7 days')";
...
const OVERWRITE_RISK_TABLE_COUNTS: &[(&str, Option<&str>)] = &[
    ...
    ("quotes", Some(SYNCABLE_QUOTES_FILTER)),
    ("goals", None),
    ("goal_plans", None),
    ("ai_threads", None),
    ("ai_messages", None),
    ...
    pub async fn restore_snapshot_tables_from_file(
        ...
        self.writer
            .exec(move |conn| {
                ...
                // (SQLite silently ignores it). The WriteHandle wraps every job in
                // immediate_transaction(), so we use PRAGMA defer_foreign_keys instead,
                // which IS allowed inside a transaction and defers FK enforcement to
                // the transaction commit. By commit time all tables are restored in FK
                // dependency order, so the final state is FK-consistent.
                diesel::sql_query(attach_sql)
                    .execute(conn)
                    .map_err(StorageError::from)?;

                let _ = diesel::sql_query("PRAGMA foreign_keys = OFF").execute(conn);
                let _ = diesel::sql_query("PRAGMA defer_foreign_keys = ON").execute(conn);
                ...
                let restore_result = (|| -> Result<()> {
                    ...
                    for table in &table_set {
                        ...
                        let columns_sql = common_columns
                            ...
                            .collect::<Vec<_>>()
                            .join(", ");
                        // For filtered tables, apply the same filter to both DELETE and INSERT
                        // so we only touch user-syncable rows and preserve system seed data.
                        let filter = snapshot_filter_for_table(table);
                        let copy_sql = match filter {
                            Some(where_clause) => format!(
                ...
                // Re-enable FK enforcement (was disabled before the restore loop).
